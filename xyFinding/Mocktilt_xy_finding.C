...
vector<float> upper_eta_value, upper_phi_value, upper_layer;
vector<float> lower_eta_value, lower_phi_value, lower_layer;  
//////////////////////////////////////////////////////////////////////
...
void find_eta_phi_layer_label(const vector<float> &gx, const vector<float> &gy, const vector<float> &gz) {
    for (int i = 0; i < gx.size(); i++) {
        if (gx[i] >= 0) {
            upper_gx.push_back(gx[i]);  upper_gy.push_back(gy[i]);  upper_gz.push_back(gz[i]);
            float phi_i = atan2(gy[i], gx[i]);
            upper_phi_value.push_back(phi_i);
            float r_i = sqrt(pow(gx[i], 2) + pow(gy[i], 2));  float z_i = gz[i] - upper_found_z;
            if (r_i >= 2.37 && r_i <= 2.80) {
                upper_layer.push_back(0);
	        }
	        else if (r_i >= 3.14 && r_i <= 3.59) {
                upper_layer.push_back(1);
	        }
	        else if (r_i >= 3.91 && r_i <= 4.3925) {
                upper_layer.push_back(2);
	        }
            else {
                upper_layer.push_back(3);
            }
            ...
            upper_eta_value.push_back(eta_i);
        }
        else {
            lower_gx.push_back(gx[i]);  lower_gy.push_back(gy[i]);  lower_gz.push_back(gz[i]);
            float phi_i = atan2(gy[i], gx[i]);
            lower_phi_value.push_back(phi_i);
            float r_i = sqrt(pow(gx[i], 2) + pow(gy[i], 2));  float z_i = gz[i] - lower_found_z;
            if (r_i >= 2.37 && r_i <= 2.80) {
                lower_layer.push_back(0);
	        }
	        else if (r_i >= 3.14 && r_i <= 3.59) {
                lower_layer.push_back(1);
	        }
	        else if (r_i >= 3.91 && r_i <= 4.3925) {
                lower_layer.push_back(2);
	        }
            else {
                lower_layer.push_back(3);
            }
            float theta_i = atan2(r_i, z_i);
            float eta_i;
            if (z_i >= 0) {
            ...
            lower_eta_value.push_back(eta_i);
        }
    }
}
 ...
 * @return ** double 
 ******************************************/
float aggregate_angular_distance (const int & evt, const vector<float> &eta_value, const vector<float> &phi_value, const vector<float> &layer) {
...
std::pair<float, float> find_x_y (const char &flag, const int & evt, const vector<float> &gx, const vector<float> &gy, const vector<float> &gz, const vector<float> &eta_value, const vector<float> &phi_value, const vector<float> &layer, const float &angular_distance_cut, const float &found_z) {
...
void Mocktilt_xy_finding (Int_t target = 0) {
    ifstream found_information_file("found_z.txt");
    if (!found_information_file.is_open()) {
        cout << "Unable to open the text file!" << endl;
		system("read -n 1 -s -p \"Press any key to continue...\" echo");
		exit(1);
    }
    //ofstream outfile("median_found_all.txt", std::ios_base::app);
    ofstream outfile("all_found_divided.txt", std::ios_base::app);
    if (!outfile.is_open()) {
    ...
    for(Long64_t i = 0; i < nentries; i++)
    {
        ...
        if (i >= target) {
            // if (size_of_event > 1100) {
                find_eta_phi_layer_label(*hitfromG4P_X, *hitfromG4P_Y, *hitfromG4P_Z);
