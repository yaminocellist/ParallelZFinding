...
double ChiTwo (const TH1D * const hDiff, const double &p1, const double &p2, const double &low_range, const double &high_range) {
...
void fit_the_hist (const std::string &rootFilePath) {
    ...
    TLine *line_horizon = new TLine(hSubtracted->GetXaxis()->GetXmin(), 0, hSubtracted->GetXaxis()->GetXmax(), 0);
...
void fit_hists_all (const std::vector<std::string>& rootFilePaths) {
    int i = 0;
    ...
    for (const std::string &rootFilePath : rootFilePaths) {
        ...
        TCanvas *can = new TCanvas(Form("can%d", i), Form("Background Subtracted Signal%d", i), 1920, 1056);
        can -> Divide(1, 2);
        can -> cd(1);
        hDiff->SetLineColor(kBlue);
        hDiff->SetLineWidth(2);
...
void dPhi_Fitter() {
    ...
    // fit_the_hist(rootFilePaths[0]);
    fit_hists_all(rootFilePaths);
