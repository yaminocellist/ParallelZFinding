...
void full()
{
    ...
    // ifstream myfile("./zFindingResults/foundZ_MBD0_10_DCA_0_16_-001_001_2e-1.txt");
    ifstream myfile("./zFindingResults/foundZ_MBD0_10_DCAfit_0_16_-001_001_2e-1.txt");
    ...
    for (Long64_t i = 0; i < EventTree->GetEntries(); ++i) {
        ...
        if (NTruthVtx == 1 && TruthPV_Npart->at(0) > 500 && j < evt.size()) {
            ...
            // std::cout << event << "," << evt[j] << "," << i << "," << idx[j] << "," << j << std::endl;
            Event = event;
            Index = i;
            foundZ_vtx = foundz[j];
            trueXfromSource = TruthPV_x->at(0);
